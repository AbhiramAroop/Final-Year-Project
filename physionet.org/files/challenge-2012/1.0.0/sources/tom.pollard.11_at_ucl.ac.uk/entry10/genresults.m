...
%     containing the known outcomes.  Change them if necessary.
        ...
        select_set = input('Enter 0 for full dataset or return for test set: ');
        if select_set == 0
            % FULL SET
            results='Outcomes-a.txt';
            set_name='set-a';
...
if(~isempty(results))
    ...
    score1 = min(Se, PPV);
    display(['  ']);
    display(['EVENT 1 (Perfect score = 1)']);
	display(['Random guessing = 0.139.']);
    display(['Sample entry = 0.296.']);
    display(['Best score = 0.531 (Ali).']);
    display(['Your score: ' num2str(score1)]);

    % The event 2 score is the Hosmer-Lemeshow statistic (H).
   	display(['  ']);
    display(['EVENT 2 (Perfect score = 0)']);;
    display(['Random guessing = 9666.']);
    display(['Sample entry = 68.']);
    display(['Best score = 14.397 (Joy).']);
    display(['Your score: ' num2str(H)]);
end
